...
export const translate = (word) => {
  const locale = global.accept_lang || 'en';
...
export const compareTwoArrays = (
  ...
  keyOne: string = null,
  keyTwo: string = null,
  lengthMustMatch = true,
): boolean => {
  if (lengthMustMatch && arrayOne.length !== arrayTwo.length) return false;

  if (keyOne && keyTwo)
    return arrayOne.every((arrOneElement) =>
      arrayTwo.find(
        (arrTwoElement) =>
          arrTwoElement && arrTwoElement[keyTwo] == arrOneElement[keyOne],
      ),
    );
  else
    return arrayOne.every((arrOneElement) =>
      arrayTwo.find((arrTwoElement) => arrTwoElement == arrOneElement),
    );
};
...
export const getImageBasePath = (): string => {
  ...
  else return process.env.AWS_URL;
};
